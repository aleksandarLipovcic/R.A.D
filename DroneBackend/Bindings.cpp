...
PYBIND11_MODULE(DroneBackend, m) {
    ...
    // ── DroneState ────────────────────────────────────────────────────────────
    // Read-only snapshot. Python never holds a lock — just reads value types.
    py::class_<DroneState>(m, "DroneState")
        ...
        // Convenience dict — roll/pitch divided by 10 so Python gets degrees
    ...
    py::class_<IMUSensor>(m, "IMUSensor")
        ...
        // Raw ADC counts — fastest path, no scaling overhead
        ...
        // Physical units — use for display and logging in the thesis GUI
        ...
        return py::dict(
            ...
            }, "Scaled data: accel in g, gyro in degrees/second.");
