...
//  10.  MSP_GPS_SV_INFO (164)  satellite list, every SV_POLL_TICKS iterations
//                              (~1 s at 100 Hz). Normal MSP query, no passthrough.
//
// The old pollSatellites() UBX passthrough path has been removed from this
// loop. sv_baud_probe.py confirmed that the GPS UART returns 0 bytes on all
// UART indices with gps_auto_config=ON (BF 4.5.x default). MSP cmd 164
// is the only working satellite data source on this hardware.
// =============================================================================
...
void DroneLink::communicationLoop() {
    ...
    while (keepRunning.load()) {
        ...
        //
        // This is a plain MSP query: no passthrough handshake, no port close,
        // no reopen delay. The ~1 ms round-trip is negligible at 10 ms budget.
        // svPollTickCounter_ counts loop iterations; at SV_POLL_TICKS it fires
        // and resets. Setting it to SV_POLL_TICKS on the first tick ensures
        // the satellite list is populated on the very first loop iteration
        // so svInfoValid becomes true within the first second of connect().
...
// the response to GPSNeoM10::parseMspSvInfo().
//
// No passthrough, no port close/reopen. The full round-trip is ~1-2 ms,
// well within the 10 ms loop budget.
//
...
//
// This function is NO LONGER called from communicationLoop(). It is kept
// because applyGPSConfig() uses the same passthrough open/close cycle and
// readUbxResponse() is a shared dependency.
//
// DO NOT re-enable this for satellite polling: sv_baud_probe.py confirmed
// that BF returns 0 bytes on all 6 UART indices with gps_auto_config=ON.
//
// If a future firmware update or hardware revision makes passthrough work,
// restore the call from communicationLoop() and set svSource = "UBX".
// =============================================================================
...
// parseMspSvInfo()
//
// DroneLink wrapper around GPSNeoM10::parseMspSvInfo(). Called from
// pollSatellitesMSP() — exists as a separate method so future unit tests
// can verify the DroneState wiring independently of the send/receive path.
// =============================================================================
